...
func createDB(mode os.FileMode, options *bbolt.Options) *boltdepot.Depot {
	// Create temporary path.
	f, _ := ioutil.TempFile("", "bolt-")
	f.Close()
	os.Remove(f.Name())

	db, err := bbolt.Open(f.Name(), mode, options)
	if err != nil {
		panic(err.Error())
	}
...
func createChallengeStore(mode os.FileMode, options *bbolt.Options) *challengestore.Depot {
	// Create temporary path.
